...
mod crypto_cmds;
mod decode_opts;
mod decode_query_cmd;
mod decode_support_cmd;
mod disasm_cmd;
mod friction;
mod mcp_decode;
...
fn server_info_json() -> Value {
    let decode_block = mcp_decode::server_info_decode_section();
    json!({
    "name": "ghidrust",
...
fn main() -> ExitCode {
    ...
    // Accept GNU-style `--flag` as `-flag` (values unchanged).
    args = decode_opts::normalize_cli_flags(&args);
    ...
    match args[0].as_str() {
        ...
        "disasm" | "disassemble" => disasm_cmd::cmd_disasm(&args[1..], json_mode),
        "decode-support" => decode_support_cmd::cmd_decode_support(&args[1..], json_mode),
        "decode-query" => decode_query_cmd::cmd_decode_query(&args[1..], json_mode),
        "bytes" | "dump" => cmd_bytes(&args[1..], json_mode),
        "rtti" => {
            let rest = &args[1..];
            if rest
                .iter()
                .any(|a| matches!(a.as_str(), "-filter" | "-name" | "-exact" | "query"))
            {
        ...
        "crypt-constants" => crypto_cmds::cmd_crypt_constants(&args[1..], json_mode),
        "recover-strings" => crypto_cmds::cmd_recover_strings(&args[1..], json_mode),
        "crypto-capabilities" => crypto_cmds::cmd_crypto_capabilities(&args[1..], json_mode),
        "decode" => crypto_cmds::cmd_decode(&args[1..], json_mode),
 ...
 "ghidrust — AI-native reverse-engineering (CodeBrowser headless)\n\n\
         Usage:\n\
           ...
           ghidrust disasm <path> [--addr <hex>] [--count N] [--skip-bad] [--linear|--flow]\n\
                     [--brief|--pretty] [--out FILE] [--json]\n\
                     (unknown/suspect bounds: --linear first; then function create; then --flow)\n\
           ghidrust decode-support [--json]\n\
           ghidrust decode-query --query NAME [--arch ARCH] [--id N] [--bytes HEX] [--json]\n\
           ...
           ghidrust crypt-constants <path> [--algo NAME] [--limit N] [--json]\n\
           ghidrust recover-strings <path> [--only stack,tight,decoded] [--no static]\n\
                     [--functions HEX[,HEX…]] [--limit N] [--json]\n\
           ghidrust crypto-capabilities <path> [--tag decrypt|encrypt|encoding] [--json]\n\
           ghidrust decode bake [-recipe JSON | -op NAME [-key|-key-hex|-iv|-iv-hex|-nonce|-nonce-hex|-counter|-mode|-encoding VALUE]…]\n\
                     (-hex|-b64|-raw|-in|-path+-addr) [--annotate-va HEX with -path]\n\
           ghidrust decode magic [-depth N] [--crib TEXT] (-hex|-b64|-raw|-in|-path+-addr) [--json]\n\
            ...
            match Project::open(&dir) {
                Ok(p) => {
                    let entry = if let Some(id) = file_id {
                        ...
                        p.active_file()
                            .cloned()
                            .or_else(|| p.meta.files.first().cloned())
                    };
                    ...
                    if json {
                        ...
                        println!(
                            ...
                            if a.is_file() {
                                ...
                                "missing — run project analyze"
                            }
                        );
                        println!(
                            "listing: {} ({})",
                            l.display(),
                            if l.is_file() { "ok" } else { "missing" }
                        );
                        println!(
                            "export: {} ({})",
                            e.display(),
                            if e.is_file() { "ok" } else { "missing" }
                        );
                    }
                    ExitCode::SUCCESS
                }
...
fn cmd_load(args: &[String], json: bool) -> ExitCode {
    ...
    match load_path(&path) {
        Ok(prog) => {
            ...
            emit_result(&v, json, out_path.as_deref(), || {
...
pub(crate) fn emit_json_helper<T: Serialize>(value: &T) {
    emit_json(value);
}

pub(crate) fn emit_result_helper<T: Serialize>(
    value: &T,
    json: bool,
    out_path: Option<&Path>,
    text: impl FnOnce(),
) -> ExitCode {
    emit_result(value, json, out_path, text)
}
 ...
 "usage: ghidrust xrefs <path> (-to HEX | -from HEX | -string FILTER | -import NAME | -calls HEX) [-encoding ascii|utf16le|all] [-skip-stubs] [-classify] [-out FILE] [-json]"
    ...
    match load_path(&path) {
        Ok(mut prog) => {
            ...
            let rows: Vec<_> = refs
                .iter()
                .map(|r| xref_row_json(r, &prog, classify))
                .collect();
            ...
            emit_result(&payload, json, out_path.as_deref(), || {
                ...
                if let Some(f) = &filter {
                    ...
                    for (_ti, e) in hits.iter().take(40) {
                        println!(" [{}] RVA {:#x} {}", e.index, e.fn_rva, e.name);
                    }
                }
            })
        }
...
fn cmd_il2cpp_meta(args: &[String], json: bool) -> ExitCode {
    let path = match args.first() {
        ...
        None => {
            eprintln!(
                "usage: ghidrust il2cpp meta <metadata.dat> [-filter SUB] [-out FILE] [-json]"
            );
            return ExitCode::from(2);
        }
    };
 ...
 [-filter SUB] [-baseline PREV.json] [-baseline-strict] [-script-json] [-out FILE] [-json]"
    ...
    match correlate(&prog, &meta) {
        Ok(mut map) => {
            ...
            if strict_fail {
                eprintln!(
                    "baseline-strict: {} moved RVAs",
                    map.build_skew.as_ref().map(|s| s.moved.len()).unwrap_or(0)
                );
                ExitCode::FAILURE
            } else {
                code
...
fn cmd_unity_inventory(args: &[String], json: bool) -> ExitCode {
    ...
    match inventory_path(&path) {
        Ok(inv) => emit_result(&inv, json, out_path.as_deref(), || {
            println!(
                "unity-inventory schema={} root={}",
                inv.schema_version, inv.root
            );
            println!(
        ...
        Ok(prog) => match prog.function_containing(va) {
            Some(f) => {
                if json {
                    ...
                    println!("{:#x} in {} [{:#x}, {:#x})", va, f.name, f.entry, f.end);
                }
                ExitCode::SUCCESS
            }
            None => {
        ...
        Ok(mut prog) => {
            ...
            if stage1 {
                ...
                match ghidrust_decomp::decompile_stage1_at(&prog, va, count, conv) {
                    Ok((d, s1)) => {
                        let sum = s1.summary();
                        let stored_end = prog.function_at(va).map(|f| f.end);
                        let decoded_end = d.blocks.iter().map(|b| b.end).max().unwrap_or(va);
                        let honesty = assess_decompile_bounds(&prog, va, stored_end, decoded_end);
                        let linear = disassemble_range_opts(&prog, va, count, true)
                            .map(|r| r.insns)
                            .unwrap_or_default();
                        let callsite_hints = collect_callsite_hints(&linear);
                        let mut obj = json!({
                        "decompile": d,
                        "resolve": resolve_meta,
                        "stage1": {
                        ...
                        if let Some(m) = obj.as_object_mut() {
                            if let Some(hf) = honesty.to_json_fields().as_object() {
                                for (k, v) in hf {
                        ...
                        if let Some(fm) = &follow_meta {
                            obj.as_object_mut()
                                .unwrap()
                                .insert("follow_stub".into(), fm.clone());
                        }
 ...
 "usage: ghidrust ghidra-headtohead <path> [-functions N] [-count N] \\\n [-ghidra DIR] [-captured JSON] [-out FILE] \\\n [-spawn-timeout SECS] [-ghidra-fn-cap N] [-json]"
    ...
    let captured = match captured_path {
        Some(cp) => match std::fs::read_to_string(&cp) {
            Ok(text) => {
                match serde_json::from_str::<Vec<ghidrust_decomp::CapturedGhidraDecompile>>(&text) {
                    Ok(v) => Some(v),
                    Err(e) => {
...
fn format_metrics_log(m: &ReBenchMetrics) -> String {
    ...
    s.push_str(&format!(
        ...
        m.decompile_cpu.backend, m.decompile_cpu.ms, m.decompile_cpu.entry, m.decompile_cpu.name
    ));
    s.push_str(&format!(
...
fn mcp_tool(t: mcp_decode::McpToolDef) -> ToolDef {
    ToolDef {
        name: t.name,
...
fn tool_defs() -> Vec<ToolDef> {
 ...
 "required": ["session_id"]
        ...
        mcp_tool(mcp_decode::disassemble_tool_def()),
        mcp_tool(mcp_decode::decode_support_tool_def()),
        mcp_tool(mcp_decode::decode_query_tool_def()),
        ToolDef {
 name: "rtti",
 ...
 "required": ["path"]
        ...
        ToolDef {
 name: "list_analyzers",
 description: "List Auto Analysis options (labels)",
 input_schema: json!({ "type": "object", "properties": {} }),
        },
        ToolDef {
 name: "analyze",
 ...
 "required": ["path"]
        ...
        ToolDef {
 name: "crypt_constants",
 description: "Scan for cryptographic constant tables (S-boxes, TEA delta, hash K, stream sigma). Optional algo filter.",
            input_schema: json!({
 "type": "object",
 "properties": {
 "path": { "type": "string" },
 ...
 "required": ["path"]
        ...
        ToolDef {
 name: "list_crypt_constants",
 description: "Alias of crypt_constants",
            input_schema: json!({
 "type": "object",
 "properties": {
 "path": { "type": "string" },
 ...
 "required": ["path"]
        ...
        ToolDef {
 name: "recover_strings",
 description: "Recover stack/tight/decoded obfuscated strings. only/no: static|stack|tight|decoded.",
            input_schema: json!({
 "type": "object",
 "properties": {
 "path": { "type": "string" },
 "only": { "type": "array", "items": { "type": "string" } },
 "no": { "type": "array", "items": { "type": "string" } },
 "functions": { "type": "array", "items": { "type": "string" }, "description": "hex VAs" },
 "limit": { "type": "integer" }
                },
 "required": ["path"]
            }),
        ...
        ToolDef {
 name: "decode_bake",
description: "Apply a byte-transform recipe (FromBase64, FromHex, XOR, XORBrute, RC4, ChaCha20Decrypt, AESDecrypt modes cbc/ecb/ctr/gcm/cfb/ofb, DESDecrypt, TripleDESDecrypt, BlowfishDecrypt, Gunzip, Inflate, ROT13, Reverse, DecodeUTF16LE).",
            input_schema: json!({
 "type": "object",
 "properties": {
 "path": { "type": "string", "description": "Required when reading via addr" },
 "addr": { "type": "string" },
 "count": { "type": "integer" },
 "input_hex": { "type": "string" },
 "input_b64": { "type": "string" },
 "recipe": { "type": "array", "description": "[{op, args}]" },
"ops": { "type": "array" },
"annotate_va": { "type": "string", "description": "EOL-comment VA; requires path and is in-memory only for a plain path load" }
                }
            }),
        },
        ToolDef {
 name: "decode_magic",
description: "Auto-detect a short peel chain (depth 1..4) maximizing printable ratio; crib filters the selected output.",
            input_schema: json!({
 "type": "object",
 "properties": {
 "path": { "type": "string" },
...
"crib": { "type": "string" }
        ...
        ToolDef {
 name: "list_crypto_capabilities",
 description: "Match encrypt/decrypt/encoding capabilities (imports, AES-NI, constant seeds).",
            input_schema: json!({
 "type": "object",
 "properties": {
 "path": { "type": "string" },
 "tag": { "type": "string", "enum": ["decrypt", "encrypt", "encoding", "hashing"] }
                },
 "required": ["path"]
            }),
        },
        ...
        let resp = match method {
            ...
            "tools/list" | "list_tools" => {
                ...
                let tools: Vec<Value> = tool_defs()
                    ...
                    .collect();
                eprintln!(
                    "mcp tools/list → {} tools (tool_surface={})",
                    tools.len(),
...
fn mcp_decode_input_bytes(args: &Value) -> Result<Vec<u8>, String> {
    if let Some(h) = args.get("input_hex").and_then(|v| v.as_str()) {
    ...
    if let Some(b) = args.get("input_b64").and_then(|v| v.as_str()) {
        let r = ghidrust_core::bake(
            b.as_bytes(),
            &[ghidrust_core::BakeOp {
                op: "FromBase64".into(),
                args: json!({}),
            }],
        );
        ...
        return Ok((0..r.output_hex.len())
            .step_by(2)
            .map(|i| u8::from_str_radix(&r.output_hex[i..i + 2], 16).unwrap())
            .collect());
    }
    let path = args
        .get("path")
        .and_then(|p| p.as_str())
        .ok_or_else(|| "provide input_hex, input_b64, or path+addr".to_string())?;
    let addr = args
        .get("addr")
        .and_then(|a| a.as_str())
        .ok_or_else(|| "missing arguments.addr when using path".to_string())?;
    ...
    prog.read_va(va, count)
        .ok_or_else(|| format!("cannot read {count} bytes at {va:#x}"))
}
    ...
    match name {
        ...
        "process_launch" => {
            let image = args
                .get("image")
                .and_then(|p| p.as_str())
                .ok_or_else(|| "missing arguments.image".to_string())?;
        ...
        "process_resume" => {
            ...
            process_resume(sid)?;
            Ok(serde_json::to_string_pretty(&json!({"ok": true, "resumed": true})).unwrap())
        }
        "process_detach" => {
            let sid = args
                .get("session_id")
        ...
        "crypt_constants" | "list_crypt_constants" => {
            let path = args
                ...
                .ok_or_else(|| "missing arguments.path".to_string())?;
            let prog = load_path_opts(path, true).map_err(|e| e.to_string())?;
            let mut hits = if prog.analysis.crypt_constants.is_empty() {
                ghidrust_core::scan_crypt_constants(&prog)
            } else {
                prog.analysis.crypt_constants.clone()
            };
            if let Some(a) = args.get("algo").and_then(|v| v.as_str()) {
                hits.retain(|h| h.algorithm.eq_ignore_ascii_case(a));
            }
            if let Some(l) = args.get("limit").and_then(|v| v.as_u64()) {
                hits.truncate(l as usize);
            }
        ...
        "recover_strings" => {
            let path = args
                ...
                .and_then(|p| p.as_str())
                .ok_or_else(|| "missing arguments.path".to_string())?;
            let mut prog = load_path_opts(path, true).map_err(|e| e.to_string())?;
            let mut opts = ghidrust_core::RecoverStringsOpts::default();
            if let Some(arr) = args.get("only").and_then(|v| v.as_array()) {
                opts.only = Some(
                    arr.iter()
            ...
            if let Some(arr) = args.get("no").and_then(|v| v.as_array()) {
                opts.no = Some(
                    arr.iter()
                        .filter_map(|v| v.as_str().map(|s| s.to_string()))
                        .collect(),
            ...
            if let Some(arr) = args.get("functions").and_then(|v| v.as_array()) {
                let mut fns = Vec::new();
                for v in arr {
            ...
            opts.limit = args
                ...
                .and_then(|v| v.as_u64())
                .map(|n| n as usize);
            let _ = ghidrust_core::run_analyzers(&mut prog, &["Find Crypt"]);
            ...
            let hits = ghidrust_core::recover_obfuscated_strings(&prog, &opts);
            Ok(serde_json::to_string_pretty(&hits).unwrap())
        }
        "decode_bake" | "decode_magic" => {
            let input = mcp_decode_input_bytes(&args)?;
            if name == "decode_magic" {
                let depth = args.get("depth").and_then(|v| v.as_u64()).unwrap_or(3) as usize;
                let crib = args.get("crib").and_then(|v| v.as_str());
                let result = ghidrust_core::magic_with_crib(&input, depth, crib);
            ...
            let recipe = args
                ...
                .cloned()
                .ok_or_else(|| "missing arguments.recipe or arguments.ops".to_string())?;
            let ops: Vec<ghidrust_core::BakeOp> =
                serde_json::from_value(recipe).map_err(|e| e.to_string())?;
            let result = ghidrust_core::bake(&input, &ops);
            let bytes: Vec<u8> = (0..result.output_hex.len())
                .step_by(2)
                .filter_map(|i| u8::from_str_radix(&result.output_hex[i..i + 2], 16).ok())
                .collect();
            let iocs = ghidrust_core::extract_iocs(&bytes);
            let annotation = if result.ok {
                match (
                    args.get("path").and_then(|v| v.as_str()),
                    args.get("annotate_va").and_then(|v| v.as_str()),
                ) {
                    (Some(path), Some(va)) => {
                        let va = parse_u64(va)?;
                    ...
                    (None, Some(va)) => Some(json!({
                        ...
                        "applied": false,
                        "persisted": false,
                        "note": "annotate_va requires path"
                    })),
                    _ => None,
                }
            } else {
        ...
        "list_crypto_capabilities" => {
            let path = args
                ...
                .ok_or_else(|| "missing arguments.path".to_string())?;
            let mut prog = load_path_opts(path, true).map_err(|e| e.to_string())?;
            let _ = ghidrust_core::run_crypto_pipeline(&mut prog);
            let tag = args.get("tag").and_then(|v| v.as_str());
            let hits = ghidrust_core::scan_crypto_capabilities(&prog, tag);
            let hits: Vec<Value> = hits
                .iter()
                .map(|hit| {
                    let mut value = serde_json::to_value(hit).unwrap_or(Value::Null);
                    let suggested_ops: Vec<String> =
                        ghidrust_core::suggest_recipe_for_hint(&hit.capability)
                            .into_iter()
                            .map(|op| op.op)
                            .collect();
                    if let Some(object) = value.as_object_mut() {
                        object.insert("suggested_ops".into(), json!(suggested_ops));
                    }
                    value
                })
                .collect();
        ...
        "decompile" => {
            ...
            match stage {
                "stage0" | "stage-0" => {
                    let d = ghidrust_decomp::decompile_at(&prog, va, count)
                        .map_err(|e| e.to_string())?;
                    let mut obj = json!({
                    "stage": "0",
                    "resolve": resolve_meta,
        ...
        "decode_support" => mcp_decode::handle_decode_support(&args),
        "decode_query" => mcp_decode::handle_decode_query(&args),
        "disassemble" => mcp_decode::handle_disassemble(&args),
        "load" | "rtti" | "analyze" => match name {
            "load" => {
                let path = if let Some(p) = args.get("path").and_then(|p| p.as_str()) {
                    ...
                    project.root.join(&entry.imported_rel).display().to_string()
                } else {
                ...
                Ok(
                    serde_json::to_string_pretty(&friction::load_json_for_prog(&prog, &identity))
                        .unwrap(),
                )
            }
            "rtti" => {
            ...
            "analyze" => {
                ...
                let use_gpu = args.get("gpu").and_then(|g| g.as_bool()).unwrap_or(false);
