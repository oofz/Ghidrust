            ...
            if json {
                ...
                for e in inv.entries.iter().take(32) {
                    println!(
                        ...
                        e.path, e.size, e.version.file_version, e.version.product_version
                    );
                }
...
pub fn cmd_process(args: &[String], json: bool) -> ExitCode {
 ...
 "usage: ghidrust process list|attach|launch|resume|detach|modules|read|resolve|regions … [-json]"
    ...
    match args[0].as_str() {
        ...
        "launch" => {
            let image = match args.get(1).filter(|s| !s.starts_with('-')) {
 ...
 "usage: ghidrust process launch <image.exe> [-args \"…\"] [-cwd DIR] [-json]"
            ...
            while i < args.len() {
                match args[i].as_str() {
                    "-args" if i + 1 < args.len() => {
                        launch_args = Some(args[i + 1].clone());
                        i += 2;
                    }
                    "-cwd" if i + 1 < args.len() => {
            ...
            match process_launch(&LaunchRequest {
                image,
                args: launch_args,
                cwd,
            }) {
        ...
        "resume" => {
            let sid = match args.get(1) {
                Some(s) => s.as_str(),
                None => {
                    eprintln!("usage: ghidrust process resume <session_id>");
                    return ExitCode::from(2);
                }
            };
            match process_resume(sid) {
                Ok(()) => {
                    println!("{{\"ok\":true,\"resumed\":true}}");
                    ExitCode::SUCCESS
                }
                Err(e) => {
                    eprintln!("error: {e}");
        ...
        "resolve" => {
            let sid = match args.get(1) {
                Some(s) => s.as_str(),
                None => {
                    eprintln!("usage: ghidrust process resolve <session_id> -module NAME -rva HEX");
                    return ExitCode::from(2);
                }
            };
            let mut module = None;
