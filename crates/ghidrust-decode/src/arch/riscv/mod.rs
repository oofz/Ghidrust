...
pub(crate) fn decode_raw(bytes: &[u8], address: u64, mode: Mode) -> Result<Instruction> {
 ...
 return Err(Error::Decode("truncated riscv instruction".into()));
        ...
        if (word & 0x03) != 0x03 {
 return Err(Error::Decode("compressed instruction without RISCV_C".into()));
        }
...
impl ArchDecode for RiscvDecoder {
 ...
 return Err(Error::Mode(format!("invalid riscv mode {:#x}", mode.bits())));
        }
        Ok(Self { mode })
    }
...
mod tests {
 ...
 // beq x10, x11, offset => example encoding
        ...
        let insn = dec.decode_one(&beq, 0x1000, &EngineOptions::default()).unwrap();
