...
fn decode32(word: u32) -> Result<(String, String, usize)> {
    ...
    match op {
        ...
        0x01 => {
            ...
            Ok(("add".into(), format!("r{dst}, r{src1}, r{src2}"), 4))
        }
        0x02 => {
            let dst = (word >> 22) & 0x1f;
            let src1 = (word >> 17) & 0x1f;
            let src2 = (word >> 12) & 0x1f;
            Ok(("sub".into(), format!("r{dst}, r{src1}, r{src2}"), 4))
        }
        0x10 => {
            let dst = (word >> 22) & 0x1f;
            let base = (word >> 17) & 0x1f;
            let disp = (word >> 5) & 0xfff;
            Ok(("ldw".into(), format!("r{dst}, [{disp}(r{base})]"), 4))
        }
        0x11 => {
            let src = (word >> 22) & 0x1f;
            let base = (word >> 17) & 0x1f;
            let disp = (word >> 5) & 0xfff;
            Ok(("stw".into(), format!("r{src}, [{disp}(r{base})]"), 4))
        }
