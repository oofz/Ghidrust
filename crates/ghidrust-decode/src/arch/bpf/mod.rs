...
fn ebpf_mnemonic(op: u8, dst: u8, src: u8, off: i16, imm: i32) -> Result<(String, String)> {
    let cls = op & 0x07;
    match cls {
        0x00 | 0x01 => {
            // LD / LDX
            ...
            if cls == 0x00 {
                ...
                return Ok((format!("ldx{sz}"), format!("r{dst}, [r{src}+{off}]")));
            }
        ...
        0x02 | 0x03 => {
            ...
            if (op & 0xe0) == 0x60 {
                if cls == 0x02 {
                    Ok((format!("st{sz}"), format!("[r{dst}+{off}], #{imm}")))
                } else {
                    Ok((format!("stx{sz}"), format!("[r{dst}+{off}], r{src}")))
                }
            } else {
        ...
        0x05 | EBPF_JMP32 => {
            ...
            match opn {
                ...
                _ => {
                    ...
                    if op & 0x08 == 0 {
                        Ok((format!("{opn}{suffix}"), format!("r{dst}, #{imm}, {off}")))
                    } else {
                        Ok((format!("{opn}{suffix}"), format!("r{dst}, r{src}, {off}")))
                    }
                }
            }
        }
