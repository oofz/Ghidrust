...
use ghidrust_core::{default_arch_mode, arch_mode_for_program, Arch, Engine, Mode, Program, SupportQuery};
...
pub fn ui_processor_selectors(
 ...
 ui.label("Raw bits:");
                ...
                if ui
                    .add(egui::DragValue::new(&mut bits).speed(1))
                    .changed()
                {
                    opts.mode = Some(bits);
                }
 ...
 format!("Architecture {} is not supported by the decode engine.", name.name()),
            );
        }
    }
}
