...
use decrypt_ui::{DecryptPaneState, DecryptTab};
...
use ghidrust_core::{
    analyzer_catalog, analyzer_supports_gpu, arch_mode_for_program, default_arch_mode,
    disassemble_at_opts, load_path, m3_tokens, recover_obfuscated_strings, set_preferred_bulk_mode,
    xrefs_from, xrefs_to, AnalysisRunReport, AnalyzerInfo, AnalyzerOutput, Arch, BulkScanMode,
    CommentKind, CryptConstantHit, CryptoCapabilityHit, FoundString, Instruction,
    ObfuscatedStringHit, Program, Project, ProjectTreeModel, RecoverStringsOpts, RttiReport,
    ThemeMode, XRef, BUILTIN_TYPES,
};
use graphs::{
...
use listing::{
    default_start_va, listing_matches, load_decode_prefs, reload, reload_for_goto,
    save_decode_prefs, ui_detail_pane, ui_options_dialog, ui_search_bar, ui_toolbar, DecodeUiOpts,
    ListingRow, ListingSearch, ToolbarAction,
};
use menu_actions::{
    ...
    stage1_pseudo_c_with_tokens, DecompStage, ListingSelection, MemoryHit, TextHit,
    STAGE0_MAX_INSNS,
};
...
impl GhidrustApp {
    ...
    pub fn compute_checksums(&mut self, mode: ChecksumMode) -> Result<(), String> {
        ...
        self.status = format!("Checksum: crc32={crc32:#010x} adler32={adler32:#010x} len={len}",);
        self.log(self.status.clone());
        Ok(())
    }
    ...
    fn reload_listing_at(&mut self, va: u64) -> Result<(), String> {
        let prog = self
            .program
            .as_ref()
            .ok_or_else(|| "no program loaded".to_string())?;
 ...
 "analyzer '{name}' panicked (GPU/validation); try again with GPU off"
    ...
    fn ensure_project_skill_wired(&mut self) {
        ...
        agent_pane::ensure_skill_for_project(&mut self.grok_pane, &root, skill_source.as_deref());
    ...
    fn start_grok_pty(&mut self, cols: u16, rows: u16, ctx: egui::Context) -> Result<(), String> {
 ...
 "• Large RTTI set ({} classes) — always filter; the list is virtualized so it stays smooth.",
            ...
            match kind {
                ...
                PaneKind::Decrypt => {
                    ...
                    win.show(ctx, |ui| {
                        ...
                        {
                            match act {
                                decrypt_ui::DecryptPaneAction::ApplyComment { va, text } => {
                                    if let Err(e) =
                                ...
                                decrypt_ui::DecryptPaneAction::ApplyBookmark { va, text } => {
                                    self.add_bookmark(va, BookmarkKind::Analysis, "decrypt", format!("decrypted: {text}"));
                                }
                                decrypt_ui::DecryptPaneAction::SendToListing { va } => {
                                    let _ = self.goto_address_str(&format!("{va:#x}"));
                                    self.status = format!("Decrypt input focused in Listing at {va:#x}; output was not written as code.");
                                }
                                decrypt_ui::DecryptPaneAction::LoadVa { va, len } => {
                                    self.open_decrypt_at_len(va, len, None);
                                }
                                decrypt_ui::DecryptPaneAction::Goto { va } => {
                                    let _ = self.goto_address_str(&format!("{va:#x}"));
                                }
                                decrypt_ui::DecryptPaneAction::DecryptNearby { va, hint } => {
                                    self.open_decrypt_nearby(va, Some(hint));
                                }
                                decrypt_ui::DecryptPaneAction::BakeRemnant { va } => {
                                    self.open_decrypt_at(va, None);
                                }
                                decrypt_ui::DecryptPaneAction::FocusFunction { va } => {
                                    self.focus_function(va);
                                }
                            }
                        }
                ...
                PaneKind::InstallInventory => {
                    win.default_size(egui::vec2(760.0, 520.0)).show(ctx, |ui| {
                        tool_panes::ui_install_inventory(
                            ui,
                ...
                PaneKind::FileSystemBrowser => {
                    win.default_size(egui::vec2(720.0, 520.0)).show(ctx, |ui| {
                        tool_panes::ui_file_system_browser(
                            ui,
                ...
                PaneKind::AnalysisArtifacts => {
                    win.default_size(egui::vec2(820.0, 520.0)).show(ctx, |ui| {
                        tool_panes::ui_analysis_artifacts(
                            ui,
    ...
    fn ui_entropy_pane(&mut self, ui: &mut egui::Ui, muted: Color32, primary: Color32) {
        ...
        let clicked_o = entropy::render_overview_strip(
            ui,
            prog,
            &samples,
            muted,
 ...
 egui::RichText::new("Click a strip to Go To that address. Green = exec, amber = writable, grey = readonly, cold-blue = low entropy, warm-red = high.").color(muted),
    ...
    /// `` — register lattice + user-set value ranges.
    ...
    fn draw_debugger_panes(&mut self, ctx: &egui::Context, muted: Color32) {
        debugger::draw_host(ctx, &mut self.debugger, muted);
    }
    ...
    fn snapshot_current_layout(&self, name: impl Into<String>) -> layouts::SavedLayout {
        ...
        debugger::snapshot_layout_flags(&self.debugger, &mut open_panes);
    ...
    fn apply_saved_layout(&mut self, layout: &layouts::SavedLayout) {
        ...
        debugger::apply_layout_flags(&mut self.debugger, &layout.open_panes);
        // Docks.
 ...
 "Select a Symbol Tree function or a Listing instruction to decompile (Stage-0 CFG → pseudo-C).",
    ...
    fn open_decrypt_at(&mut self, va: u64, hint: Option<String>) {
        self.open_decrypt_at_len(va, 256, hint);
    }

    fn open_decrypt_at_len(&mut self, va: u64, len: usize, hint: Option<String>) {
        let bytes = self
            .program
            .as_ref()
            .and_then(|p| decrypt_ui::bytes_at(p, va, len))
            .unwrap_or_default();
    ...
    fn open_decrypt_nearby(&mut self, va: u64, hint: Option<String>) {
        let start = va.saturating_sub(32);
        let bytes = self
            .program
            .as_ref()
            .and_then(|p| decrypt_ui::bytes_at(p, start, 256))
            .unwrap_or_default();
        self.decrypt_pane.load_bytes(Some(va), &bytes, hint);
    ...
    fn recover_strings_at_function(&mut self, va: u64) {
        let Some(prog) = self.program.as_ref() else {
    ...
    fn ui_crypto_constants_pane(&mut self, ui: &mut egui::Ui, muted: Color32) {
        ...
        decrypt_ui::ui_crypto_constants(
            ui,
            muted,
            &hits,
            |va| goto = Some(va),
    ...
    fn ui_recovered_strings_pane(&mut self, ui: &mut egui::Ui, muted: Color32) {
        let hits = self.obfuscated_strings.clone();
        let mut goto = None;
        let mut bake = None;
        let mut decoder = None;
        decrypt_ui::ui_recovered_strings(
            ui,
            muted,
            &hits,
            |va| goto = Some(va),
    ...
    fn ui_crypto_capabilities_pane(&mut self, ui: &mut egui::Ui, muted: Color32) {
        let hits = self.crypto_capabilities.clone();
        let mut goto = None;
        let mut decrypt = None;
        decrypt_ui::ui_crypto_capabilities(
            ui,
            muted,
            &hits,
            |va| goto = Some(va),
    ...
    fn ui_relocation_table(&mut self, ui: &mut egui::Ui, muted: Color32) {
        ...
        ui.small(
            egui::RichText::new("RelocationTablePlugin · from Program::sections").color(muted),
        );
        ui.separator();
        let Some(prog) = self.program.as_ref() else {
            ui.weak("No program loaded.");
...
impl eframe::App for GhidrustApp {
            ...
            egui::menu::bar(ui, |ui| {
                ui.menu_button("File", |ui| {
                    ...
                    if ui
                        ...
                        .on_hover_text("Not yet implemented — PDB load UI")
                        .clicked()
                    {
                    ...
                    if ui
                        .button("Configure…")
                        .on_hover_text("Plugin picker — not yet implemented")
                        .clicked()
                    {
                ...
                ui.menu_button("Edit", |ui| {
                    ...
                    if ui
                        .button("Tool Options…")
                        .on_hover_text("Options tree — not yet implemented")
                        .clicked()
                    {
                    ...
                    if ui
                        ...
                        .on_hover_text("Program-level options — not yet implemented")
                        .clicked()
                    {
                    ...
                    if ui
                        .button("Plugin Path…")
                        .on_hover_text("Configure plugin paths — not yet implemented")
                        .clicked()
                    {
                    ...
                    if ui
                        .button("Key Bindings…")
                        .on_hover_text("Rebind actions — not yet implemented")
                        .clicked()
                    {
                ...
                ui.menu_button("Analysis", |ui| {
                    ...
                    if ui
                        .button("Analyze All Open…")
                        .on_hover_text("Multi-program batch — not yet implemented")
                        .clicked()
                    {
 ...
 self.tool_panes.gpu_decompile.max_bytes_input = "256".into();
                ...
                ui.menu_button("Search", |ui| {
                    ...
                    if ui
                        .button("For Matching Instructions…")
                        .on_hover_text("Not yet implemented")
                        .clicked()
                    {
                ...
                ui.menu_button("Select", |ui| {
                    ...
                    ui.separator();
                    if ui
                        .button("Create Table From Selection")
                        .on_hover_text("Not yet implemented")
                        .clicked()
                    {
                ...
                ui.menu_button("Tools", |ui| {
                    ...
                    if ui
                        .button("Function Bit Patterns Explorer")
                        .on_hover_text("Not yet implemented")
                        .clicked()
                    {
 ...
 _ => "Debugger live backend not yet wired",
                        };
                            ...
                            match act {
                                ...
                                DebuggerAction::ToggleBreakpoint => {
                                    if let Some(va) = self.listing_focus_va {
                                        ...
                                        self.nyi("Debugger → Toggle Breakpoint (no listing VA)");
                                    }
                                }
 ...
 fills this list when demangling produces `::` scopes.",
        ...
        if self.show_configure_dialog {
            ...
            egui::Window::new("Configure plugins")
                ...
                .show(ctx, |ui| {
                    ui.label(
 "Ghidrust plugins are compile-time; this dialog lists every provider Ghidrust ships.",
                    );
                    ui.separator();
                    egui::ScrollArea::vertical()
...
mod tests {
    ...
    fn script_manager_catalog_is_populated_from_mcp_surface() {
        ...
        assert!(app
            ...
            .catalog
            .iter()
            .any(|s| s.name() == "mcp.list_functions"));
    }
    ...
    fn graph_debugger_configure_menu_actions_wired_not_nyi() {
        ...
        assert!(src.contains("open_launch_ui"));
