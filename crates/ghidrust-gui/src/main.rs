...
use ghidrust_core::{
    ...
    disassemble_at_opts, load_path, m3_tokens,
    ...
    RttiReport, ThemeMode, XRef, Arch, BUILTIN_TYPES,
};
use listing::{
    default_start_va, listing_matches, reload, reload_for_goto, ui_detail_pane, ui_options_dialog,
    ui_search_bar, ui_toolbar, DecodeUiOpts, ListingRow, ListingSearch, ToolbarAction,
    load_decode_prefs, save_decode_prefs,
};
 ...
 *self.pane_open.get(&kind).unwrap_or(&false)
 ...
 ui.heading("Entropy");
        ui.small(
            egui::RichText::new(
 "EntropyPlugin analog · Shannon bits/byte over 256-byte windows",
            )
            .color(muted),
        );
        ui.separator();
 ...
 *k,
 ...
 ui.heading("Bookmarks");
        ui.small(
 egui::RichText::new("BookmarkPlugin analog · 5 standard kinds").color(muted),
        );
        ui.separator();
        ui.horizontal(|ui| {
 ui.label("Filter:");
 ...
 ui.heading("Relocation Table");
        ui.small(
 egui::RichText::new("RelocationTablePlugin · from Program::sections")
                .color(muted),
        );
        ui.separator();
        let Some(prog) = self.program.as_ref() else {
 ...
 ui.heading("Disassembled View");
        ui.small(
 egui::RichText::new("DisassembledViewPlugin · detail at cursor")
                .color(muted),
        );
        ui.separator();
        let Some(prog) = self.program.as_ref() else {
 ...
 ui.heading("Defined Data");
        ui.small(
 egui::RichText::new("DataWindowPlugin · session data (Program::data_items when available)")
                .color(muted),
        );
        ui.separator();
 ...
 ui.heading("Bytes");
        ui.small(
            egui::RichText::new(
 "ByteViewerPlugin · split hex / ASCII, follows Listing cursor",
            )
            .color(muted),
        );
        ui.separator();
 ...
 ui.heading("Symbol References");
        ui.small(
            egui::RichText::new(
 "SymbolTablePlugin · every reference TO the current symbol",
            )
            .color(muted),
        );
        ui.separator();
 ...
 ui.heading("Equates Table");
        ui.small(
            egui::RichText::new(
 "EquateTablePlugin · symbolic names bound to scalar operands",
            )
            .color(muted),
        );
        ui.separator();
 ...
 ui.heading("Function Tags");
        ui.small(
            egui::RichText::new(
 "FunctionTagPlugin · per-function labels + universe of tags",
            )
            .color(muted),
        );
        ui.separator();
 ...
 ui.heading("External Programs");
        ui.small(
            egui::RichText::new(
 "ReferencesPlugin · analyzer-derived imports + exports",
            )
            .color(muted),
        );
        ui.separator();
 ...
 ui.heading("Data Type Preview");
        ui.small(
            egui::RichText::new(
 "DataTypePreviewPlugin · byte interpretation @ cursor",
            )
            .color(muted),
        );
        ui.separator();
 ...
 ui.heading("Checksum Generator");
        ui.small(
            egui::RichText::new(
 "ComputeChecksumsPlugin · CRC-32 / Adler-32 / Fletcher / sums",
            )
            .color(muted),
        );
        ui.separator();
...
impl eframe::App for GhidrustApp {
 ...
 self.nyi("File → Export Program");
                    ...
                    ui.separator();
 if ui.button("Load PDB File…").on_hover_text("Not yet implemented — PDB load UI").clicked() {
 ...
 if ui.button("For Scalars…").on_hover_text("ScalarSearchPlugin · operand scalar range filter").clicked() {
                        self.show_search_scalars_dialog = true;
                        ui.close_menu();
                    }
 if ui.button("For Address Tables…").on_hover_text("AutoTableDisassemblerPlugin · pointer table candidates").clicked() {
 ...
 if ui.button("Instruction Patterns…").on_hover_text("BytePatternPlugin · mnemonic + operand filter").clicked() {
 ...
 ui.label(egui::RichText::new("All Providers (catalog)").small().weak());
 ...
 if ui.button("API Help").on_hover_text("Not yet implemented").clicked() {
 ...
 ui.heading("Program Trees");
                    ui.small(
 egui::RichText::new("ProgramTreePlugin · modules/fragments")
                            .weak(),
                    );
                    ui.separator();
                    let Some(prog) = self.program.as_ref() else {
