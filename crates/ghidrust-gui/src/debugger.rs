...
impl DebuggerState {
 ...
 /// After a successful attach: modules → seed memory → regions → focus Modules.
    pub fn refresh_after_attach(&mut self) {
        ...
        if let Some(main) = pick_main_module(&self.modules_cache, self.process_display_name.as_deref())
        {
 ...
 modules: &'a [ModuleInfo],
    ...
    if let Some(pname) = process_name {
        ...
        if let Some(m) = modules
            .iter()
 .find(|m| m.name.to_ascii_lowercase().contains(pl.trim_end_matches(".exe")))
        {
            return Some(m);
        }
    }
...
pub fn snapshot_layout_flags(
    ...
    for p in DebuggerPane::ALL {
        open_panes.insert(p.egui_id().to_string(), state.host_open && state.active_tab == *p);
    }
}
...
pub fn render_pane_body(pane: DebuggerPane, state: &mut DebuggerState, ui: &mut Ui, muted: Color32) {
...
fn render_modules(state: &mut DebuggerState, ui: &mut Ui, muted: Color32) {
 ...
 if ui.small_button("Memory").on_hover_text("Read Memory Bytes at module base").clicked()
                            {
 ...
 if ui.button("Resolve").clicked() {
            ...
            match static_to_live(
                &sess.session_id,
                state.static_map_module_input.trim(),
                rva,
            ) {
                Ok(r) => {
...
fn render_memory_bytes(state: &mut DebuggerState, ui: &mut Ui, muted: Color32) {
 ...
 ui.weak("No read yet — enter a VA + size and click Read.");
    ...
    let mod_subtitle = state.module_for_va(read.va).map(|m| {
        format!(
 "{}+{:#x}",
            m.name,
            read.va.saturating_sub(m.base)
        )
    });
    ui.small(format!(
 "va={:#x}{} requested={} read={}",
...
mod tests {
 ...
 assert_eq!(s.watches[0], "rbx");
    ...
    fn pick_main_module_prefers_process_name() {
        let mods = vec![
 modinfo("ntdll.dll", 0x7ffe0000, 0x1000, Some(r"C:\Windows\ntdll.dll")),
 ...
 s.modules_cache = vec![modinfo("game.exe", 0x140000000, 0x1000, Some(r"C:\game.exe"))];
        s.active_tab = DebuggerPane::Modules;
        assert_eq!(s.active_tab, DebuggerPane::Modules);
        assert!(!s.modules_cache.is_empty());
        assert_eq!(
